...
#include "../interface/MassSolver.h"
...
bool MassSolver::solve( const TVector2 & met, const TLorentzVector & bq1 ,
	const TLorentzVector & bq2 , const TLorentzVector & lep1, const TLorentzVector & lep2, 
...
//////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::quartic(vector<double> const & polx, vector<double> *pnux, int& cubic_single_root_cmplx) {
    vector<double> polxt(polx.size());
    cubic_single_root_cmplx = 0;
    if (polx[4] == 0.)
        cubic(polx, pnux);
    else {
        for (unsigned int i=0; i<polxt.size(); ++i) {
      ...
      //cout << "polxt[" << i << "]=" << polxt[i] << endl;
        ...
        if (polxt[0] == 0.) {
            ...
            for (unsigned int i=0; i<4; ++i) polxt2[i] = polxt[i+1];
            cubic(polxt2, pnux);
        }
        else {
      ...
      //cout << "e=" << e << " f=" << f << " g=" << g << endl;
            if (g == 0.) {
                ...
                polxt2[3]=1.;
                cubic(polxt2, pnux);
                for (unsigned int i=1; i<(*pnux).size(); ++i)
                    (*pnux)[i]-=polxt[3]/4.;	
            }
            else if (f == 0.) {
                ...
                for (unsigned int i=0; i<polxt3.size(); ++i) {
                    if (polxt3[i]>=0.) {
	  ...
	  //cout <<"Warning: quartic: single cubic h^2 solution is negative! (unique sol=" << polxt3[0] << ") => complex root h" << endl;
		...
		if (h < 0) return;
                ...
                for (unsigned int i=0; i<polxt5.size(); ++i)
                ...
                for (unsigned int i=0; i<polxt6.size(); ++i)
...
//////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::cubic(vector<double> const & polx, vector<double> *pnux) {
    if (polx[3] == 0.)
        quadratic(polx, pnux);
    else {
        ...
        if (q == 0.) { //single root of degree three
            (*pnux).push_back(sign(-r)*pow(fabs(2.*r),1./3.)-polx[2]/3.);
        }
    ...
    return;
        ...
        //////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::quadratic(vector<double> const & polx, vector<double> *pnux) {
...
int MassSolver::algebraic_pz(double* b, double* lp, double mWp, double mt, 
                               ...
                               double* pnuz)
{
  ...
  //determine first temporary pnuz, pnubz
    ...
    if (radicant>=0.) {
        ...
        cout << "MassSolver: algebraic_pz: pnuz_a radicant=" << radicant 
    ...
    //cout << "pnuz_b solutions= " << b1val+sqrt(radicant) << " and " << b1val-sqrt(radicant) << endl;
        ...
        cout << "MassSolver: algebraic_pz: pnuz_b radicant=" << radicant 
                << " should never happen!" << endl;
    }
    ...
    for (unsigned int j=0; j<pnuz_a.size(); ++j) {
        for (unsigned int k=0; k<pnuz_b.size(); ++k) {
    ...
    return 0; //success
                               ...
                               /////////////////////////////////////////////////////////////////
double MassSolver::evalterm1(vector<double> *a1, double pnux, double pnuy) {
