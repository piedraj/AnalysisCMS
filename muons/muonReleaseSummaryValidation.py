...
def GetLabel(params):
    ...
    if (params['PileUp']!='no'):
        if (params['Condition']!='PUpmx50ns_MCRUN2' and params['Condition']!='PUpmx25ns_MCRUN2'):
...
def getSampleFiles(params, sample):
    ...
    checkFile = path + '/general_tpToTkmuAssociation' + GetFastSimSuffix(params)+'.png'
    ...
    if (os.path.isfile(localsample)==True):
        ...
        if (params['Condition'].find('100X')!=-1 or params['Condition'].find('94X')!=-1 or params['Condition'].find('MCRUN2')!=-1 or params['Condition'].find('mcRun2')!=-1):
            if (userparams.NewParams['FastSim']| userparams.RefParams['FastSim']):
                if ((sample=="RelValTTbar")|(sample=="RelValZMM")|(sample=="RelValUpsilon1SToMuMu")):
                ...
                if( (sample=="RelValZpMM_13TeV") | (sample=="RelValWpM_13TeV")):
                    print "Subfix not added"     
                    theGuiPostFixLS1 = ""
                elif ((sample=="RelValTTbar")|(sample=="RelValZMM")|(sample=="RelValUpsilon1SToMuMu")|(sample=="RelValWpM")|(sample=="RelValZpMM")):
                    #theGuiPostFixLS1 = "_UPGpostls1_14"
                    theGuiPostFixLS1 = "_13"
                else:
                    if (params['Condition'].find('mcRun2')!=-1):
                        theGuiPostFixLS1 = ""
                    else:
                        theGuiPostFixLS1 = ""
        ...
        guiFileName=params['DQM']+'__'+theGuiSample+'__'+params['Release']+'-'+GetLabel(params)+'__'+params['Format']+'.root'
        guiFullURL=GetGuiRepository(params)+guiFileName
#        guiFileName = 'afs/cern.ch/work/l/lveldere/public/muonvalidation/new_dqm.root'
        ...
        if (downloadfile(guiFullURL)==True):
                localpath=GetSamplePath(params, sample)
		print('   + Copying ' + guiFileName + ' to ' + localpath)
#        	shutil.copy(guiFileName,localsample)
...
def getReplaceMap(newparams, refparams, sample, datatype, cfgkey, cfgfile):
    ...
    replace_map = { 'DATATYPE': datatype,
                    'NEW_FILE': guiFileNameNew,
                    'REF_FILE': guiFileNameRef,
                    'REF_LABEL':sample,
                    'NEW_LABEL': sample,
                    'REF_RELEASE':refparams['Release_c'],
                    'NEW_RELEASE':newparams['Release_c'],
...
for sample in userparams.samples :
    ...
    guiFileNameNew=getSampleFiles(userparams.NewParams, sample)
    guiFileNameRef=getSampleFiles(userparams.RefParams, sample)
#    guiFileNameNew ='/afs/cern.ch/work/l/lveldere/public/muonvalidation/new_dqm.root'
#    guiFileNameRef ='/afs/cern.ch/work/l/lveldere/public/muonvalidation/old_dqm.root'
