					   ...
					   int ijet)
{
  ...
  //----------------------------------------------------------------------------
  float deltarjet1met  = -0.1;
  float deltarjet2met  = -0.1;
  float deltarjj       = -0.1;
  float deltarjjmet    = -0.1;
  float deltarlep1jet1 = -0.1;
  float deltarlep1jet2 = -0.1;
  float deltarlep2jet1 = -0.1;
  float deltarlep2jet2 = -0.1;
  float deltarllmet    = -0.1;
  float deltarl1met    = -0.1;
  float deltarl2met    = -0.1;
                                               
  // Fill DeltaR variables
  //----------------------------------------------------------------------------
  deltarllmet = fabs((Lepton1.v+Lepton2.v).DeltaR(MET));
  deltarl1met = fabs(Lepton1.v.DeltaR(MET));
  deltarl2met = fabs(Lepton2.v.DeltaR(MET));

  if (njet > 0)
    {
      deltarjet1met  = fabs(AnalysisJets[0].v.DeltaR(MET));
      deltarlep1jet1 = fabs(Lepton1.v.DeltaR(AnalysisJets[0].v));
      deltarlep2jet1 = fabs(Lepton2.v.DeltaR(AnalysisJets[0].v));
    }

  if (njet > 1)
    {
      deltarjet2met  = fabs(AnalysisJets[1].v.DeltaR(MET));
      deltarlep1jet2 = fabs(Lepton1.v.DeltaR(AnalysisJets[1].v));
      deltarlep2jet2 = fabs(Lepton2.v.DeltaR(AnalysisJets[1].v));
      deltarjj       = fabs(AnalysisJets[0].v.DeltaR(AnalysisJets[1].v));
      deltarjjmet    = fabs((AnalysisJets[0].v + AnalysisJets[1].v).DeltaR(MET));
    }
  ...
  h_deltarjet1met   [ichannel][icut][ijet]->Fill(deltarjet1met,    _event_weight);
  h_deltarjet2met   [ichannel][icut][ijet]->Fill(deltarjet2met,    _event_weight);
  h_deltarjj        [ichannel][icut][ijet]->Fill(deltarjj,         _event_weight);
  h_deltarjjmet     [ichannel][icut][ijet]->Fill(deltarjjmet,      _event_weight);                         
  h_deltarlep1jet1  [ichannel][icut][ijet]->Fill(deltarlep1jet1,   _event_weight);                                                                             
  h_deltarlep1jet2  [ichannel][icut][ijet]->Fill(deltarlep1jet2,   _event_weight);                                                                              
  h_deltarlep2jet1  [ichannel][icut][ijet]->Fill(deltarlep2jet1,   _event_weight);                                                                              
  h_deltarlep2jet2  [ichannel][icut][ijet]->Fill(deltarlep2jet2,   _event_weight);  
